import { describe, expect, it } from 'vitest';
import { evaluateTrigger } from '../src/triggers/index.js';
import { OverrideMeta, PackageJson } from '../src/types.js';

const dateMeta = (expires: string): OverrideMeta => ({
  reason: 'foo bar',
  owner: 'John Doe',
  revisitWhen: { type: 'date', expires },
});

const anchorMeta = (packageName: string, declaredRange: string): OverrideMeta => ({
  reason: 'foo bar',
  owner: 'John Doe',
  revisitWhen: { type: 'version-anchor', package: packageName, declaredRange },
});

const patchHashMeta = (hash: string): OverrideMeta => ({
  reason: 'backport upstream fix',
  owner: 'team-foo',
  revisitWhen: { type: 'patch-hash', hash },
});

const withPackageJson = (packageJson: PackageJson) => ({ packageJson });

describe('evaluateTrigger', () => {
  describe('date trigger', () => {
    it('should not fire when expires is in the future', () => {
      const result = evaluateTrigger(
        dateMeta('2099-01-01'),
        withPackageJson({}),
        new Date('2026-05-25'),
      );
      expect(result.fired).toBe(false);
      expect(result.reason).toBe('Expires on 2099-01-01');
    });

    it('should fire when expires is in the past', () => {
      const result = evaluateTrigger(
        dateMeta('2020-01-01'),
        withPackageJson({}),
        new Date('2026-05-25'),
      );
      expect(result.fired).toBe(true);
      expect(result.reason).toBe('Expired on 2020-01-01');
    });

    it('should fire when expires equals today (boundary)', () => {
      const result = evaluateTrigger(
        dateMeta('2026-05-25'),
        withPackageJson({}),
        new Date('2026-05-25'),
      );
      expect(result.fired).toBe(true);
    });
  });

  describe('version-anchor trigger', () => {
    it('should not fire when the declared range still matches dependencies', () => {
      const result = evaluateTrigger(
        anchorMeta('foo', '^1.2.0'),
        withPackageJson({ dependencies: { foo: '^1.2.0' } }),
      );
      expect(result.fired).toBe(false);
      expect(result.reason).toBe('foo still pinned at ^1.2.0');
    });

    it('should fire when the declared range has changed', () => {
      const result = evaluateTrigger(
        anchorMeta('foo', '^1.2.0'),
        withPackageJson({ dependencies: { foo: '^2.0.0' } }),
      );
      expect(result.fired).toBe(true);
      expect(result.reason).toBe('foo declared range changed: ^1.2.0 -> ^2.0.0');
    });

    it('should fire when the dependency has been removed entirely', () => {
      const result = evaluateTrigger(
        anchorMeta('foo', '^1.2.0'),
        withPackageJson({ dependencies: { bar: '^1.0.0' } }),
      );
      expect(result.fired).toBe(true);
      expect(result.reason).toBe('foo is no longer a dependency');
    });

    it('should look up the package in devDependencies', () => {
      const result = evaluateTrigger(
        anchorMeta('foo', '^1.2.0'),
        withPackageJson({ devDependencies: { foo: '^1.2.0' } }),
      );
      expect(result.fired).toBe(false);
    });

    it('should look up the package in peerDependencies', () => {
      const result = evaluateTrigger(
        anchorMeta('foo', '^1.2.0'),
        withPackageJson({ peerDependencies: { foo: '^1.2.0' } }),
      );
      expect(result.fired).toBe(false);
    });

    it('should treat semver-equivalent ranges as unchanged', () => {
      const result = evaluateTrigger(
        anchorMeta('foo', '1.2.0'),
        withPackageJson({ dependencies: { foo: '=1.2.0' } }),
      );
      expect(result.fired).toBe(false);
    });

    it('should ignore whitespace differences in equivalent ranges', () => {
      const result = evaluateTrigger(
        anchorMeta('foo', '^1.2.0'),
        withPackageJson({ dependencies: { foo: '  ^1.2.0  ' } }),
      );
      expect(result.fired).toBe(false);
    });

    it('should treat unparseable ranges as changed when they differ as strings', () => {
      const result = evaluateTrigger(
        anchorMeta('foo', 'latest'),
        withPackageJson({ dependencies: { foo: '^1.2.0' } }),
      );
      expect(result.fired).toBe(true);
    });

    it('should treat identical unparseable ranges as unchanged via string fast-path', () => {
      const result = evaluateTrigger(
        anchorMeta('foo', 'latest'),
        withPackageJson({ dependencies: { foo: 'latest' } }),
      );
      expect(result.fired).toBe(false);
    });

    it('should fire when no dependency sections are present at all', () => {
      const result = evaluateTrigger(
        anchorMeta('foo', '^1.2.0'),
        withPackageJson({ name: 'empty' }),
      );
      expect(result.fired).toBe(true);
      expect(result.reason).toBe('foo is no longer a dependency');
    });
  });

  describe('patch-hash trigger', () => {
    it('should not fire when the stored hash matches the computed hash', () => {
      const result = evaluateTrigger(patchHashMeta('sha256:abc123'), {
        packageJson: {},
        patchContentHash: 'sha256:abc123',
      });
      expect(result.fired).toBe(false);
      expect(result.reason).toBe('Patch content matches recorded hash');
    });

    it('should fire when the stored hash differs from the computed hash', () => {
      const result = evaluateTrigger(patchHashMeta('sha256:abc123'), {
        packageJson: {},
        patchContentHash: 'sha256:def456',
      });
      expect(result.fired).toBe(true);
      expect(result.reason).toContain('Patch content changed');
    });

    it('should fire when patchContentHash is undefined (patch file deleted)', () => {
      const result = evaluateTrigger(patchHashMeta('sha256:abc123'), withPackageJson({}));
      expect(result.fired).toBe(true);
      expect(result.reason).toBe('Patch file no longer present');
    });

    it('should ignore packageJson when evaluating patch-hash triggers', () => {
      const result = evaluateTrigger(patchHashMeta('sha256:abc123'), {
        packageJson: { dependencies: { foo: '^1.0.0' } },
        patchContentHash: 'sha256:abc123',
      });
      expect(result.fired).toBe(false);
    });
  });
});
