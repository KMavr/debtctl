...
- **No install** — `npx debtctl@0.2.0 check`. Pinned to a version, fetched on demand. Fine for one-off exploration; slower on every CI run than installing.
...
`debtctl` is at `0.2.0`. The metadata schema is versioned (`"version": 1`); future schema changes will bump it explicitly. Bug reports and PRs welcome.
